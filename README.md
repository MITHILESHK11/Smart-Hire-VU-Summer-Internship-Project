...
### Frontend (Vercel)
Deploying the static Vite application to Vercel is extremely easy:
...
*The frontend `vercel.json` already contains custom rules to ensure clean URL routing for the SPA.*

### Backend (Docker & Cloud hosting)
Because the backend executes heavy neural network models (SBERT/PyTorch), it is best deployed on services that support Docker containers or persistent servers.

1. **Docker Compose**:
   To run both backend and frontend locally in production mode using docker:
   ```bash
   docker-compose up --build
   ```
2. **Cloud Containers (Render / Railway / VPS)**:
   - Deploy using the provided `backend/Dockerfile`.
   - Ensure the server has at least **2GB RAM** to handle Python ML model loading in memory.
